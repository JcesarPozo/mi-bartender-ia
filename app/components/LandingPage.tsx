...
const DEMO_COCKTAILS = [
  { name: 'Mojito Maracuyá', emoji: '🌴', desc: 'Ron blanco, maracuyá fresco, menta, lima y soda. Tropical y vibrante.', tags: ['Tropical', 'Cítrico', 'Ron'], img: 'https://www.thecocktaildb.com/images/media/drink/metwgh1606770327.jpg' },
  { name: 'Negroni Ahumado', emoji: '🔥', desc: 'Gin ahumado, Campari, vermut rojo. Intenso, sofisticado y memorable.', tags: ['Clásico', 'Amargo', 'Gin'], img: 'https://www.thecocktaildb.com/images/media/drink/qgdu971561574065.jpg' },
  { name: 'Blue Velvet',     emoji: '💙', desc: 'Blue Curaçao, zumo de piña, agua tónica. Visualmente impactante.',    tags: ['Tropical', 'Dulce', 'Licor'], img: 'https://www.thecocktaildb.com/images/media/drink/bry4qh1582751040.jpg' },
];
...
const TESTIMONIALS = [
  { quote: 'Le pedí "algo para una noche de verano con la persona que me gusta" y me dio la receta más perfecta que he probado en mi vida.', author: 'Elena M.', cocktail: 'Rosa de Medianoche', avatar: '🌹' },
  { quote: 'Organicé una cena para 12 personas y cada uno pidió su cóctel personalizado. Todos fliparon. Nadie se creyó que lo había hecho yo solo.', author: 'Carlos R.', cocktail: 'Velvet Storm', avatar: '🍾' },
  { quote: 'Soy bartender profesional y lo uso para inspirarme. Las combinaciones que propone son originales de verdad, no las típicas de siempre.', author: 'Sofía K.', cocktail: 'Obsidian Sour', avatar: '🥂' },
];
...
const TICKER_ITEMS = ['Espresso Martini', 'Paloma de Hibisco', 'Negroni Blanco', 'Mojito de Fresa', 'Dark & Stormy', 'Clover Club', 'Velvet Sour', 'Midnight Rose', 'Jungle Bird', 'Paper Plane', 'Last Word', 'Aperol Twist', 'Frozen Margarita', 'Amaretto Dreams', 'Blue Lagoon', 'Golden Hour'];

const fadeUp = { hidden: { opacity: 0, y: 36 }, visible: (i = 0) => ({ opacity: 1, y: 0, transition: { delay: i * 0.1, duration: 0.7, ease: [0.22, 1, 0.36, 1] as any } }) };
...
export default function LandingPage({ onGetStarted }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-[#000810] text-white overflow-x-hidden selection:bg-[#f5c842]/30">
      ...
      {/* ── NAV ── */}
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#000810]/85 backdrop-blur-xl border-b border-[#f5c842]/8' : ''}`}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <motion.span animate={{ rotate: [0, -8, 8, 0] }} transition={{ duration: 3, repeat: Infinity, repeatDelay: 5 }} className="text-2xl inline-block">🍸</motion.span>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {[['Experiencia', '#experiencia'], ['Creaciones', '#demo'], ['Bartenders', '#bartenders']].map(([label, href]) => (
              <a key={label} href={href} className="text-sm text-[#f5c842]/55 hover:text-[#f5c842] transition-colors tracking-wide">{label}</a>
            ))}
          </nav>
          <div className="flex items-center gap-3">
            <button onClick={onGetStarted} className="text-sm text-[#f5c842]/65 hover:text-[#f5c842] transition-colors px-3 py-1.5">Entrar</button>
            <motion.button whileHover={{ scale: 1.03, boxShadow: '0 0 24px rgba(245,200,66,0.25)' }} whileTap={{ scale: 0.97 }} onClick={onGetStarted}
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16" style={{ position: 'relative' }}>
        ...
        <motion.div style={{ y: heroY, opacity: heroOpacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, scale: 0.3, rotate: -20 }} animate={{ opacity: 1, scale: 1, rotate: 0 }} transition={{ duration: 1, ease: [0.22, 1, 0.36, 1] }}
            className="text-8xl md:text-[9rem] mb-10 inline-block filter drop-shadow-[0_0_60px_rgba(245,200,66,0.3)]">🍸</motion.div>

          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25, duration: 0.8 }}
            className="text-xs uppercase text-[#f5c842]/45 mb-7 font-medium tracking-[0.35em]">Tu mixólogo personal con inteligencia artificial</motion.p>

          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35, duration: 0.9 }} className="font-serif font-bold leading-[1.04] mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.6 }}
            ...
            <motion.button whileHover={{ scale: 1.04, boxShadow: '0 0 50px rgba(245,200,66,0.35)' }} whileTap={{ scale: 0.97 }} onClick={onGetStarted}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 1 }}
            ...
            {[{ value: '12.400+', label: 'Cócteles creados' }, { value: '3.200+', label: 'Bartenders' }, { value: '4.8 ★', label: 'Valoración media' }].map(s => (
      ...
      <div className="border-y border-[#f5c842]/8 py-4 overflow-hidden bg-[#000c1f]/40">
        <motion.div animate={{ x: ['0%', '-50%'] }} transition={{ duration: 28, repeat: Infinity, ease: 'linear' }} className="flex gap-12 whitespace-nowrap">
      ...
      <section id="experiencia" className="py-36 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-24">
            ...
            <motion.h2 variants={fadeUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight">De la idea al vaso<br /><span className="text-[#f5c842]">en 10 segundos</span></motion.h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-[#f5c842]/8 rounded-3xl overflow-hidden">
            {[{ n: '01', icon: '💬', title: 'Describe el momento', desc: '"Algo tropical para la terraza", "clásico y elegante para cenar", "sin alcohol pero con carácter". La IA te entiende.' }, { n: '02', icon: '✨', title: 'La IA lo inventa', desc: 'Receta única. Proporciones exactas. Técnica de preparación. Historia del cóctel. Imagen generada al momento.' }, { n: '03', icon: '🚀', title: 'Guarda y comparte', desc: 'Tu catálogo personal crece. Comparte la tarjeta del cóctel en redes. Cada receta, tuya para siempre.' }].map((step, i) => (
              <motion.div key={step.n} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.18, duration: 0.7 }}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-serif text-[#f5c842]/15 text-7xl font-bold leading-none group-hover:text-[#f5c842]/25 transition-colors">{step.n}</span>
                </div>
      ...
      <section id="demo" className="py-24 px-6 bg-[#000c1f]/50">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-16">
            ...
            <motion.h2 variants={fadeUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight">Cada receta,<br /><span className="text-[#f5c842]">única como tú</span></motion.h2>
          </motion.div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div className="flex flex-col gap-3">
              {DEMO_COCKTAILS.map((cocktail, i) => (
                <motion.button key={cocktail.name} onClick={() => setActiveDemo(i)} whileHover={{ x: 6 }}
                  className={`text-left p-6 rounded-2xl border transition-all duration-300 ${activeDemo === i ? 'border-[#f5c842]/45 bg-[#f5c842]/7 shadow-lg shadow-[#f5c842]/5' : 'border-[#f5c842]/8 hover:border-[#f5c842]/22 bg-[#000810]/40'}`}>
                  <div className="flex items-center gap-3 mb-2">
                    <span className="text-2xl">{cocktail.emoji}</span>
                    <span className={`font-serif font-bold text-xl transition-colors ${activeDemo === i ? 'text-[#f5c842]' : 'text-white/80'}`}>{cocktail.name}</span>
                    {activeDemo === i && <motion.span layoutId="active-dot" className="ml-auto w-2 h-2 rounded-full bg-[#f5c842]" />}
                  </div>
                  <p className="text-sm text-white/45 ml-9 leading-relaxed">{cocktail.desc}</p>
                  <div className="flex gap-2 mt-3 ml-9">
                    {cocktail.tags.map(t => <span key={t} className="text-[10px] px-2.5 py-0.5 rounded-full border border-[#f5c842]/12 text-[#f5c842]/45 tracking-wide">{t}</span>)}
                  </div>
                </motion.button>
              ))}
            </div>
            <div className="relative aspect-square rounded-3xl overflow-hidden border border-[#f5c842]/12 shadow-2xl shadow-black/50">
              <AnimatePresence mode="wait">
                <motion.img key={c.img} src={c.img} alt={c.name} initial={{ opacity: 0, scale: 1.06 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }} transition={{ duration: 0.55 }} className="w-full h-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/default-cocktail.jpg'; }} />
              </AnimatePresence>
              <div className="absolute inset-0 bg-gradient-to-t from-[#000810]/85 via-[#000810]/10 to-transparent" />
              <div className="absolute bottom-5 left-5 right-5">
                <AnimatePresence mode="wait">
                  <motion.div key={c.name} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.35 }}>
              ...
              <div className="absolute top-5 right-5 flex gap-1.5">
                {DEMO_COCKTAILS.map((_, i) => <button key={i} onClick={() => setActiveDemo(i)} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === activeDemo ? 'bg-[#f5c842]' : 'bg-white/20 hover:bg-white/40'}`} />)}
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-32 px-6 bg-[#000c1f]/50">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-20">
            ...
            <motion.h2 variants={fadeUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight">No es solo una receta.<br /><span className="text-[#f5c842]">Es una experiencia.</span></motion.h2>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((f, i) => (
              <motion.div key={f.title} variants={fadeUp} custom={i} whileHover={{ y: -5, borderColor: 'rgba(245,200,66,0.28)' }}
      ...
      <section id="bartenders" className="py-36 px-6">
        <div className="max-w-5xl mx-auto">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="text-center mb-20">
            ...
            <motion.h2 variants={fadeUp} className="font-serif text-4xl md:text-6xl font-bold leading-tight">Lo que dicen<br /><span className="text-[#f5c842]">nuestros bartenders</span></motion.h2>
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <motion.div key={t.author} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.7 }}
                ...
                <p className="text-white/70 text-sm leading-relaxed -mt-6">{t.quote}</p>
                <div className="flex items-center gap-3 pt-2 border-t border-[#f5c842]/8 mt-auto">
                  <div className="w-10 h-10 rounded-full bg-[#f5c842]/10 flex items-center justify-center text-xl">{t.avatar}</div>
                  <div>
      ...
      <section className="py-40 px-6 text-center relative overflow-hidden">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={stagger} className="relative z-10 max-w-3xl mx-auto">
          <motion.div variants={fadeUp} className="text-8xl mb-10 inline-block filter drop-shadow-[0_0_40px_rgba(245,200,66,0.4)]">🍸</motion.div>
          <motion.h2 variants={fadeUp} className="font-serif text-5xl md:text-7xl font-bold mb-8 leading-[1.04]">
            <span className="text-white">El cóctel perfecto</span><br /><span className="text-[#f5c842]">te espera.</span>
          </motion.h2>
          <motion.p variants={fadeUp} className="text-lg text-white/50 mb-12 leading-relaxed">
            Únete a más de 3.000 bartenders que ya están<br className="hidden md:block" />creando recetas que no existen en ningún libro.
          </motion.p>
          <motion.button variants={fadeUp} whileHover={{ scale: 1.04, boxShadow: '0 0 70px rgba(245,200,66,0.35)' }} whileTap={{ scale: 0.97 }} onClick={onGetStarted}
            ...
            <span>Inventar mi cóctel ahora</span><span className="text-xl">→</span>
          </motion.button>
        </motion.div>
      </section>
      ...
      <footer className="border-t border-[#f5c842]/8 py-12 px-6">
        <div className="max-w-5xl mx-auto flex flex-col items-center gap-8">
          ...
          {/* ─── Donación ─────────────────────────────────────────── */}
          <DonationButton locale="es" variant="full" dark />

          <div className="w-full flex flex-col md:flex-row items-center justify-between gap-4 pt-4 border-t border-[#f5c842]/8">
            <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-3">
              <img src="/logo-borrachos.jpg" alt="Borrach@s y más" className="h-8 rounded-lg opacity-50" />
